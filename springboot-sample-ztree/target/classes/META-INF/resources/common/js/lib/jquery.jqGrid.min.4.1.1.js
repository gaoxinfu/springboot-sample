...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		imgs = "<span class='s-ico' style='float:left;display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+iac+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+dir+"'></span>";
		imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+idc+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+dir+"'></span></span>";
		for(i=0;i<this.p.colNames.length;i++){
			var tooltip = ts.p.headertitles ? (" title=\""+$.jgrid.stripHtml(ts.p.colNames[i])+"\"") :"";
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+">";
			idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
			thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+">"+imgs+ts.p.colNames[i];
...
$.jgrid.extend({
	...
	setAdjustGridWidth : function(source,adjustTarget,adjustWidth,shrinkToFit)
	{
		return this.each(function(){
			var $t = this;
			var currentWidth = ($("#"+adjustTarget).width()-adjustWidth);
			if($("#"+adjustTarget).width() > 0 && $t.grid.width != currentWidth)
			{
			 source.jqGrid('setGridWidth',$("#"+adjustTarget).width()-adjustWidth,shrinkToFit);
			}
		});
	},
