...
JSONEditor.prototype.getText = function () {
    return JSON2.stringify(this.json);
};
...
TreeEditor.prototype.getText = function() {
    return JSON2.stringify(this.get());
};
...
TextEditor.prototype._create = function (container, options, json) {
    ...
    buttonFormat.title = '格式化JSON数据, 包含具体的json格式';
    ...
    buttonCompact.title = '缩进json编码，去除全部空格';
...
TextEditor.prototype.compact = function () {
    ...
    this.setText(JSON2.stringify(json));
};
...
TextEditor.prototype.format = function () {
    var json = util.parse(this.getText());
    this.setText(JSON2.stringify(json, null, this.indentation));
};
...
Node.prototype.setValue = function(value, type) {
    ...
    if (this.type == 'array') {
        ...
        /* TODO
         ...
         var escValue = JSON2.stringify(value);
...
Node.prototype.getDom = function() {
    ...
    if (this.editor.mode.edit) {
        ...
        if (this.parent) {
            ...
            domDrag.title = '拖拽改字段 (Alt+Shift+Arrows)';
...
// titles with explanation for the different types caopeng
Node.TYPE_TITLES = {
    'auto': '字段类型为 "auto". ' +
        '自动判断字段类型 ' +
        '可能是 string, number, boolean, or null.',
    'object': '字段类型为 "object". ' +
        '对象类型包含 key/value 类型.',
    'array': '字段类型为 "array". ' +
        '数组类型包含集合类型.',
...
Node.prototype.showContextMenu = function (anchor, onClose) {
            ...
            'submenu': [
                {
                    'text': '升序',
                    'className': 'sort-asc',
                    'title': '设定当前类型下子元素 ' + this.type + ' 为升序排序',
                    'click': function () {
                        node._onSort('asc');
                    }
                },
                {
                    'text': '降序',
                    'className': 'sort-desc',
                    'title': '设定当前类型下子元素 ' + this.type + ' 为降序排序',
                    'click': function () {
                        node._onSort('desc');
                    }
                }
    ...
    if (this.parent && this.parent._hasChilds()) {
        ...
        if (node == childs[childs.length - 1]) {
            items.push({
                'text': '之后添加',
                'title': '添加(选中之后)新的类型元素，默认为 \'auto\' 类型 (Ctrl+Shift+Ins)',
                'submenuTitle': '在当前选中的元素下添加',
                'className': 'append',
                'click': function () {
                    node._onAppend('', '', 'auto');
                },
        ...
        items.push({
        	  'text': '之前添加',
            'title': '在当前选中字段之前添加一个字段，默认为 \'auto\' 类型 (Ctrl+Ins)',
            'submenuTitle':'在当前选中的元素之前添加',
            'className': 'insert',
            'click': function () {
...
Node.prototype._escapeHTML = function (text) {
    ...
    var json = JSON2.stringify(htmlEscaped);
...
AppendNode.prototype.showContextMenu = function (anchor, onClose) {
    ...
    var items = [
        ...
        {
            'text': '添加',
            'title': '添加字段类型默认为 \'auto\' (Ctrl+Shift+Ins)',
            'submenuTitle': '选中字段下添加',
            'className': 'insert',
            'click': function () {
                node._onAppend('', '', 'auto');
            },
...
function SearchBox (editor, container) {
    ...
    searchPrevious.title = '上一个结果 (Shift+Enter)';
